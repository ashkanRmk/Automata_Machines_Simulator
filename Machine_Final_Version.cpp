...
public:
		...
		if (mf.is_open())
		{
			...
			while (mf.good())
			{
				...
				if (str == "")
					continue;
				if (str[0] == '!')
					blank = str.erase(0, 1)[0];
				if (str[0] == '^')
					curState = str.erase(0, 1);
				if (str[0] == '>')
					input = str.erase(0, 1);
			}
			mf.close();
	...
	{
		vector<state_utm>::iterator f = states.begin();
		while (true)
		{
...
private:
	...
	vector<state_utm> states;
	char blank;
	string curState;
	string input;

	void parseState(string str, state_utm& stt)
	{
		string a[5];
		int idx = 0;
...
void UTM_Menu()
{
	...
	gotoxy(25, 4);
	color(11);
	cout << "Universal Turing Machine Simulater";
	gotoxy(10, 8);
	color(15);
	cout << "Input your Universal Turing in |Turing.txt| file with this form:";
	gotoxy(8, 12);
	color(11);
	cout << "<current state> <current symbol> <new symbol> <direction> <new state>\n";
	color(15);
